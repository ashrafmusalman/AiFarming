...
class AddProductRepo(
                ...
                task.storage.downloadUrl.addOnSuccessListener { url ->
                    ...
                    firebaseFirestore.collection("FarmingProduct")
                        ...
                        .addOnSuccessListener {
                            Log.d("Product", "Product Added" + product.title)
    ...
    fun fetchOthersListedProduct() {
        ...
        Log.d("FetchProducts", "fetchOthersListedProduct() called — currentUid = $currentUid")

        // Fetch ALL products from collection then filter locally
        firebaseFirestore.collection("FarmingProduct")
            .get()
            .addOnSuccessListener { snapshot ->
                Log.d("FetchProducts", "raw snapshot size = ${snapshot.size()}")
                val docs = snapshot.documents
                Log.d("FetchProducts", "document ids: ${docs.map { it.id }}")

                // convert to objects and filter client-side
                val allProducts = snapshot.toObjects(Product::class.java)
                val filtered = if (currentUid != null) {
                    allProducts.filter { it.sellerId != currentUid }
                } else {
                    // if no user logged in, return all products (or decide your policy)
                    allProducts
                }

                Log.d("FetchProducts", "after filtering: ${filtered.size} items")
                _fetchOthersListedProductStatus.value = Resources.Success(filtered)
            }
            .addOnFailureListener { e ->
                Log.e("FetchProducts", "fetchOthersListedProduct ERROR: ${e.localizedMessage}")
                _fetchOthersListedProductStatus.value = Resources.Error(e.localizedMessage)
            }
    }
