...
import com.ashraf.farming.viewmodel.AddPostViewmodel
...
class AddPostFragment : Fragment() {
    ...
    private fun observeAddPost() {
        lifecycleScope.launch {// create the new couroutine
            addPostViewmodel.postStatus.collect {
                when (it) {
                    is Resources.Error -> {// if there is some error
                        progressDialog.hide()
                    ...
                    is Resources.Success -> {// if data is available to user
                        progressDialog.hide()
                        Toast.makeText(
                            requireContext(),
