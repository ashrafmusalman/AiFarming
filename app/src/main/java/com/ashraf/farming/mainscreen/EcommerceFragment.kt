...
import com.ashraf.farming.adapter.CatgoryApdater
import com.ashraf.farming.adapter.OtherProductAdapter
import com.ashraf.farming.datamodel.Product
import com.ashraf.farming.util.Constant.Companion.CATEGORY_LIST
            ...
            addProductViewmodel.fetchOtherProductStatus.collect {
                when (it) {
                    ...
                    is Resources.Success -> {
                        ...
                        it.data?.let { product ->
                            productList = product
                            Log.d("observeOtherProduct", "Submitting list to adapter: ${product.size} items")
                            otherProductAdapter.differ.submitList(product)
                        }
                        if (it.data.isNullOrEmpty()) {
                            Log.d("observeOtherProduct", "Received empty list from repo")
                        }
                        binding.progressBar.visibility = View.GONE
                    }
